      ...
      <div className="w-full max-w-xs flex mt-20">
        <div className="bg-white shadow-md rounded p-10 mb-4">
          <div className="mb-4">
            ...
            />
            <div className="text-red-600">{errorUsername && errorUsername}</div>
          </div>
          <div className="mb-4">
            <label
              className="block text-gray-700 text-sm font-bold mb-2"
            ...
            <input
              ...
              placeholder="Email"
            />
            <div className="text-red-600">{errorEmail && errorEmail}</div>
          </div>
          <div className="mb-4">
            <label
              className="block text-gray-700 text-sm font-bold mb-2"
            ...
            <input
              ...
              placeholder="******************"
            />
            <div className="text-red-600">{errorPassword && errorPassword}</div>
          </div>
          <div className="flex items-center justify-between">
            <button
              ...
              onClick={async () => {
                ...
                let messages = ValidateRegister(username, email, password);
                setErrorUsername(messages["username"]);
                setErrorEmail(messages["email"]);
                setErrorPassword(messages["password"]);
                if (Object.keys(messages).length == 0) {
                  try {
                    const u = await RegisterAuth(email, password);
                    await updateProfile(u.user, {
                      displayName: username,
                    });
                    await AddUser(u.user.uid, username, email, password);
                    navigate("/");
