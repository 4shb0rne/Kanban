...
export const AttachCardLabel = async (boardId, cardId, labelId) => {
    await updateDoc(doc(db.getDB(), `boards/${boardId}/cards/${cardId}`), {
        labelId: doc(db.getDB(), `boards/${boardId}/cardlabels`, labelId),
    });
};
...
export const setCardDueDate = async (boardId, cardId, date) => {
    const card = await getDoc(
        doc(db.getDB(), `boards/${boardId}/cards`, cardId)
    );
    ...
    await addDoc(collection(db.getDB(), `boards/${boardId}/events`), {
        ...
        await updateDoc(
            doc(db.getDB(), `boards/${boardId}/events`, docRef.id),
            {
                id: docRef.id,
            }
        );
